...
describe Performance::Metrics::PeapMetricSender do
  let(:today) { Date.today }
  let(:s3_client) { Performance::Metrics.fake_s3_client }
  let(:test_logger) { double("logger", info: nil, warn: nil) }

  subject(:monthly_rolling_peap) do
    described_class.new(period: "day", date: today, metric: :monthly_rolling_peap, logger: test_logger)
  end

  subject(:month_to_date_peap) do
    described_class.new(period: "day", date: today, metric: :month_to_date_peap, logger: test_logger)
  end
  ...
  it "defaults to a standard logger writing to $stdout" do
    sender = described_class.new(period: "day", date: today, metric: :monthly_rolling_peap)
  ...
  describe "#to_s3" do
    ...
    it "sends 'monthly rolling PEAP users' data to S3 in the expected folder and filename" do
      monthly_rolling_peap.to_s3
      key = "service-report-peap-unique-users-rolling-count/service-report-peap-unique-users-rolling-count-#{today}"
      expect(s3_contents(key)).to eq(monthly_rolling_expected_hash)
    end

    it "sends 'month to date PEAP users' data to S3 in the expected folder and filename" do
      month_to_date_peap.to_s3
      key = "service-report-peap-unique-users-mtd-count/service-report-peap-unique-users-mtd-count-#{today}"
      expect(s3_contents(key)).to eq(month_to_date_expected_hash)
    end

    context "when stats are nil" do
      before do
        allow_any_instance_of(Performance::UseCase::MonthlyRollingWindowPeapUniqueUsers)
  ...
  describe "#to_api" do
    ...
    let(:monthly_rolling_expected_api_payload) do
      {
        "name" => "service-report-peap-unique-users-rolling-count",
        "value" => "0",
    ...
    it "sends rolling PEAP stats payload to the metrics API" do
      captured = nil
      ...
      monthly_rolling_peap.to_api

      expect(captured).to eq(monthly_rolling_expected_api_payload)
    end

    it "sends MTD PEAP stats payload to the metrics API" do
      captured = nil
      stub_request(:post, expected_api_url).with do |req|
        captured = JSON.parse(req.body)
        true
