...
describe Performance::UseCase::MonthToDatePeapUniqueUsers do
  ...
  context "with PEAP users connecting once this month" do
    before do
  ...
  context "with multiple connections by the same PEAP user this month" do
    before do
      month_start = Date.new(today.year, today.month, 1)
  ...
  context "with PEAP connections before the current month" do
    before do
      previous_month_end = Date.new(today.year, today.month, 1) - 1

      sessions.insert(
        siteIP: "10.0.0.1",
        username: "USER01",
        start: previous_month_end,
        success: 1,
      )
    end

    it "ignores sessions before the 1st of the month" do
      expect(result[:count]).to eq(0)
    end
  end
