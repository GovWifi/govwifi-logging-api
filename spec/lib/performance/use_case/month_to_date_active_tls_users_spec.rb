# frozen_string_literal: true

describe Performance::UseCase::MonthToDateActiveTlsUsers do
  let(:sessions) { DB[:sessions] }
  let(:result) { subject.fetch_stats }
  let(:today) { Date.today }

  subject { described_class.new(period: "day", date: today) }

  before do
    sessions.truncate
  end

  context "with TLS users connecting once this month" do
    before do
      # Use the 1st of current month
      month_start = Date.new(today.year, today.month, 1)

      sessions.insert(
        siteIP: "10.0.0.1",
        username: nil,
        cert_serial: "SERIAL123",
        cert_issuer: "GovWifi CA",
        start: month_start,
        success: 1,
      )

      sessions.insert(
        siteIP: "10.0.0.2",
        username: nil,
        cert_serial: "SERIAL456",
        cert_issuer: "GovWifi CA",
        start: today,
        success: 1,
      )
    end

    it "counts both distinct TLS users" do
      expect(result).to eq(
        count: 2,
        metric_name: "service-report-active-tls-user-mtd-count",
        period: "day",
        date: today.to_s,
      )
    end
  end

  context "with multiple connections by the same TLS user this month" do
    before do
      month_start = Date.new(today.year, today.month, 1)

      sessions.insert(
        siteIP: "10.0.0.1",
        username: nil,
        cert_serial: "SERIAL123",
        cert_issuer: "GovWifi CA",
        start: month_start,
        success: 1,
      )

      sessions.insert(
        siteIP: "10.0.0.2",
        username: nil,
        cert_serial: "SERIAL123",
        cert_issuer: "GovWifi CA",
        start: today,
        success: 1,
      )
    end

    it "counts each TLS user only once" do
      expect(result[:count]).to eq(1)
    end
  end

  context "with non-TLS sessions where username is present" do
    before do
      sessions.insert(
        siteIP: "10.0.0.1",
        username: "user01",
        cert_serial: "SERIAL123",
        cert_issuer: "GovWifi CA",
        start: today,
        success: 1,
      )
    end

    it "ignores sessions with a non-null username" do
      expect(result[:count]).to eq(0)
    end
  end

  context "with failed TLS authentication sessions" do
    before do
      sessions.insert(
        siteIP: "10.0.0.1",
        username: nil,
        cert_serial: "SERIAL123",
        cert_issuer: "GovWifi CA",
        start: today,
        success: 0,
      )
    end

    it "ignores unsuccessful sessions" do
      expect(result[:count]).to eq(0)
    end
  end

  context "with TLS connections before the current month" do
    before do
      # Last day of previous month
      previous_month_end = Date.new(today.year, today.month, 1) - 1

      sessions.insert(
        siteIP: "10.0.0.1",
        username: nil,
        cert_serial: "SERIAL123",
        cert_issuer: "GovWifi CA",
        start: previous_month_end,
        success: 1,
      )
    end

    it "ignores sessions before the 1st of the month" do
      expect(result[:count]).to eq(0)
    end
  end

  context "with an empty sessions table" do
    it "returns 0" do
      expect(result).to eq(
        count: 0,
        metric_name: "service-report-active-tls-user-mtd-count",
        period: "day",
        date: today.to_s,
      )
    end
  end
end
